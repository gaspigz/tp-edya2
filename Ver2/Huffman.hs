...
-- Importamos de esta forma para que 
-- se pueda diferenciar el uso de la
-- función empty de Maps y Heaps
...
{-
  Para este trabajo se considera que 2 árboles de Huffman
  son iguales si tienen el mismo peso. Esta decisión parte
  de que en el trabajo no usamos o comparamos 2 árboles 
  de Huffman para ver si son iguales o no, más bien usamos
  la instancia de Eq para árboles de Huffman para definir 
  su instancia en Ord. Y como la instancia en Ord está 
  definida a partir de los pesos, decidimos hacer lo mismo
  para la instancia de Eq.
  Esto nos permite no tener que definir todas las funciones 
  en la instancia de Ord.
-}
...
-- Funcion auxiliar para build_FreqMap
...
-- build_FreqMap: Recibe un string y computa la cantidad de apariciones de 
